...
# READ DATA
# ------------------------------------------------------------------------------
ds <- read_excel("C:/Users/corn0187/OneDrive - Flinders/R evaluation/Manuscript - Rubric/Stage 3 Data/Section4_evaluation.xlsx")
...
# ==============================================================================
# Figure 2 D) Automated generation of physician-to-physician clinical documentation
# =============================================================================
...
domain_summary <- domain_summary %>%
  mutate(
    Model = factor(Model, levels = c("GPT-5 Auto (via ChatGPT)", "Claude Opus 4.1 (via Claude.ai)")),
...
domain_summary$Domain_Model <- factor(
  ...
  levels = as.vector(t(outer(domain_order, c("Claude Opus 4.1 (via Claude.ai)","GPT-5 Auto (via ChatGPT)"), paste, sep = " - ")))
)
...
ggplot(domain_summary,
  ...
  scale_x_discrete(
    labels = c(
      ...
      "Claude Opus 4.1 (via Claude.ai)" = "Claude Opus 4.1"
    )
  ) +
  ...
  coord_flip() +
  labs(
    title = "D) Automated generation of physician-to-physician clinical documentation",
    x = NULL,
    y = "Percentage",
    fill = "Response"
  ) +
...
# =====================================================================================================
# Chi-Square and Fisher's Exact Test Results of Broad Agreement categories (excluding NA’s)— Scenario 4
# =====================================================================================================

run_tests <- function(df) {
  tbl <- df %>%
    filter(!is.na(Response_broad)) %>%  # Exclude NA responses
...
# ===========================================================================================
# Healthcare scenario 4 - Distribution of Assessor Response by domain (Supplementary Figures)
# ===========================================================================================
...
# Filter for Claude Opus 4.1 (via Claude.ai)
ds_claude <- ds_long %>% filter(Model == "Claude Opus 4.1 (via Claude.ai)")
...
# ===== PLOT 2: Claude Opus 4.1 (via Claude.ai) =====
plot_claude <- ggplot(ds_claude, aes(x = Response, fill = Assessor)) +
  geom_bar(position = position_dodge2(preserve = "single", padding = 0.1), 
           color = "black", width = 0.8) +
  ...
  scale_y_continuous(breaks = scales::pretty_breaks(n = 5), 
                     sec.axis = dup_axis()) +
  labs(
    title = "Distribution of Assessor Response by Domain - Claude Opus 4.1 (via Claude.ai)",
    x = "Rating",
    y = "Count",
    fill = "Assessor"
  ) +
...
# ===========================================================================================
# Healthcare scenario 4 - Inter rater agreement (Gwet and Pairwise)
# ===========================================================================================
...
# ==============================================================================
# Scenario 4: Cramér's V + Benjamini-Hochberg correction
...
View(chi_results_scenario4)

# ---- Report the actual family size used --------------------------------------
cat("Testable tests (family size m):", sum(!is.na(chi_results_scenario4$Chi.p.value)), "\n")
cat("Non-testable (degenerate tables):",
    sum(is.na(chi_results_scenario4$Chi.p.value)), "\n\n")

cat("Nominally significant (p < 0.05):",
    sum(chi_results_scenario4$Chi.p.value < 0.05, na.rm = TRUE), "\n")
cat("Significant after BH (q < 0.05):",
    sum(chi_results_scenario4$p_adj_BH < 0.05, na.rm = TRUE), "\n")

write.xlsx(chi_results_scenario4,
           "C:/Users/corn0187/OneDrive - Flinders/R evaluation/Manuscript - Rubric/Stage 3 Data/Section4_chi_BH.xlsx")
